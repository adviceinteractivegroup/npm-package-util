...
const getBestResult = ({ scores, query, client }) => {
  ...
  let results = scores.filter(business => {
    console.log(`nameSorensen: ${business.nameSorensenScore}`);
    console.log(`nameLeven: ${business.nameLevenshteinScore}`);
    console.log(`addr: ${business.addrScore}`);
    console.log(`phone: ${business.phoneScore}`);
    if (
      business.link || 

      // phone similar, address sorta close, name not so much
      (business.nameSorensenScore > 0.2 && business.addrScore >= 0.60 && business.phoneScore === 1) ||

      (business.nameSorensenScore >= 0.55 || business.nameLevenshteinScore < 15) && business.phoneScore === 1 ||
    ...
    parseAddress(`${client.street1}${client.street2}, ${client.city}, ${client.state}, ${client.postal}`)
      ...
      .then(parsedResults => {
        ...
        parsedResults.forEach(result => {
          if (`${client.postal}`.replace(" ", "") === `${result.parseAddress.postal_code}`.replace(" ", "")) {
