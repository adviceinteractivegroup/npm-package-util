...
const calculateBusinessScores = (client, parsedClientAddr, directory) => {
  ...
  directory.nameSorensenScore = sorensen(
    ...
    filterStopWords(directory.name.decodeURLpart().toLowerCase()).replace(/[,\.\-]/g, ' ').replace(/\s+/g, ' ').trim());
...
const getBestResult = (scores, query) => {
  ...
  let results = scores.filter(business => {
    ...
    (business.nameSorensenScore >= 0.55 || business.nameLevenshteinScore < 10) && business.phoneScore >= 0.8 || 
  ...
  return new Promise((resolve, reject) => {
    ...
    parseAddress(`${client.street1}${client.street2}, ${client.city}, ${client.state}, ${client.postal}`)
      ...
      .then(directoryBusiness => {
        let bestResult = getBestResult(directoryBusiness, query);
        if(bestResult) {
          resolve(bestResult);
        } else{
          reject({});
